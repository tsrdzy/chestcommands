...
<script>
import "@/../public/js/MinecraftColorCodes.3.7";
...
export default {
  ...
  mounted() {
    this.size = this.$store.state.menu["menu-settings"].rows;
    var data = this.$store.state.menu["menu-settings"].name;
  ...
  methods: {
    ...
    card_click(x, y) {
      this.x = x;
      this.y = y;
      this.$store.commit("xy", [x, y]);
    },
    item_versions() {
      this.items = require("@/../public/json/1." + this.item_version + ".json");
      require("@/../public/img/1." + this.item_version + ".css");
    },
    mousemoves(event, xy) {
    ...
    d1() {
      const object_data = JSON.parse(JSON.stringify(this.$store.state.menu));

      if (this.$store.state.menu_file != "") {
        var filename = this.$store.state.menu_file + ".yml";
      } else {
    ...
    d2() {
      ...
      var xx = this.$store.state.x;
      var yy = this.$store.state.y;
      if (xx != null && yy != null) {
        var r = confirm("是否清空当前位置!");
        if (r == true) {
          this.$store.commit("delete_card", [xx, yy]);
        }
      } else {
        alert("当前选中位置为空,无法清空!");
      }
    ...
    d3() {
      ...
      var yy = this.$store.state.y;
      if (xx != null && yy != null) {
        this.$store.commit("paste_card", [xx, yy, "item-" + xx + "-" + yy]);
      } else {
        alert("你还没有选择位置,无法粘贴");
      }
    },
    d4() {
      ...
      var yy = this.$store.state.y;
      if (xx != null && yy != null) {
        this.$store.commit("copy_card", [xx, yy]);
      } else {
        alert("你还没有选择位置,无法复制");
      }
    },
  ...
  computed: {
    rows() {
      return this.$store.state.menu["menu-settings"].rows;
    },
    name() {
      return this.$store.state.menu["menu-settings"].name;
    },
    menusdata() {
