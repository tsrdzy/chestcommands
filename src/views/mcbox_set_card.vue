...
<script>
import cc_actions from "@/../public/json/actions.json";
import cc_enchantment from "@/../public/json/enchantment.json";
import { NPopover } from "naive-ui";
import mcbox_item_selection from "@/views/mcbox_item_selection.vue";

import enchantment_all from "@/../public/json/enchantment.json";
export default {
  components: {
    NPopover,
    mcbox_item_selection,
  ...
  methods: {
    ...
    item_commont() {
      ...
      if (
        this.item_name != "" &&
        this.$store.state.x != null &&
        this.$store.state.y != null &&
        ...
        this.$store.commit("card_exist", [this.item_name, this.item_cards]);
      } else {
        alert("位置,标签,id不可为空!");
      }
    },
  ...
  watch: {
    ...
    xy(newVal) {
      ...
      if (Object.keys(this.$store.state.menu).length == 1) {
        //判断当前有无子控件
        //如果没有,生成位置标签
        this.item_name =
          "item-" + this.$store.state.x + "-" + this.$store.state.y;
      } else {
        for (let key in newVal[2]) {
          ...
          if (key != "menu-settings") {
            //排除主控件
            if (
              this.$store.state.menu[key]["POSITION-X"] ==
                this.item_cards["POSITION-X"] &&
              this.$store.state.menu[key]["POSITION-Y"] ==
                this.item_cards["POSITION-Y"]
            ) {
              this.item_name = key;
              this.item_cards.MATERIAL = this.$store.state.menu[key].MATERIAL;
              this.temporary.NAME = this.$store.state.menu[key].NAME;
              if (
                this.$store.state.menu[key].LORE != undefined &&
                this.$store.state.menu[key].LORE != ""
              ) {
                for (
                  var k = 0;
                  k < this.$store.state.menu[key].LORE.length;
                  k++
                ) {
                  if (this.$store.state.menu[key].LORE[k] == "") {
                    if (this.temporary.LORE != "") {
                    ...
                    this.temporary.LORE =
                      this.temporary.LORE +
                      "\n" +
                      this.$store.state.menu[key].LORE[k];
                  }
                }
                this.temporary.LORE = this.temporary.LORE.substr(1);
              }
              this.temporary.DURABILITY =
                this.$store.state.menu[key].DURABILITY;
              this.temporary.AMOUNT = this.$store.state.menu[key].AMOUNT;
              this.temporary["NBT-DATA"] =
                this.$store.state.menu[key]["NBT-DATA"];
              this.temporary.COLOR = this.$store.state.menu[key].COLOR;
              this.temporary["SKULL-OWNER"] =
                this.$store.state.menu[key]["SKULL-OWNER"];
              this.temporary["BANNER-COLOR"] =
                this.$store.state.menu[key]["BANNER-COLOR"];
              if (
                this.$store.state.menu[key]["BANNER-PATTERNS"] != undefined &&
                this.$store.state.menu[key]["BANNER-PATTERNS"] != ""
              ) {
                for (
                  var i = 0;
                  i < this.$store.state.menu[key]["BANNER-PATTERNS"].length;
                  i++
                ) {
                  if (this.$store.state.menu[key]["BANNER-PATTERNS"][i] == "") {
                    if (this.temporary["BANNER-PATTERNS"] != "") {
                    ...
                    this.temporary["BANNER-PATTERNS"] =
                      this.temporary["BANNER-PATTERNS"] +
                      "," +
                      this.$store.state.menu[key]["BANNER-PATTERNS"][i];
                  }
                }
                this.temporary["BANNER-PATTERNS"] =
                  this.temporary["BANNER-PATTERNS"].substr(1);
              }
              this.temporary.PRICE = this.$store.state.menu[key].PRICE;
              this.temporary.LEVELS = this.$store.state.menu[key].LEVELS;
              if (
                this.$store.state.menu[key]["REQUIRED-ITEMS"] != undefined &&
                this.$store.state.menu[key]["REQUIRED-ITEMS"] != ""
              ) {
                for (
                  var n = 0;
                  n < this.$store.state.menu[key]["REQUIRED-ITEMS"].length;
                  n++
                ) {
                  if (this.$store.state.menu[key]["REQUIRED-ITEMS"][n] == "") {
                    if (this.temporary["REQUIRED-ITEMS"] != "") {
                    ...
                    this.temporary["REQUIRED-ITEMS"] =
                      this.temporary["REQUIRED-ITEMS"] +
                      "," +
                      this.$store.state.menu[key]["REQUIRED-ITEMS"][n];
                  }
                }
                this.temporary["REQUIRED-ITEMS"] =
              ...
              this.temporary["KEEP-OPEN"] =
                this.$store.state.menu[key]["KEEP-OPEN"];
              this.temporary.PERMISSION =
                this.$store.state.menu[key].PERMISSION;
              this.temporary["VIEW-PERMISSION"] =
                this.$store.state.menu[key]["VIEW-PERMISSION"];
              this.temporary["PERMISSION-MESSAGE"] =
                this.$store.state.menu[key]["PERMISSION-MESSAGE"];
              if (this.$store.state.menu[key].ENCHANTMENTS == undefined) {
                this.temporary.ENCHANTMENTS = [];
              } else {
                this.temporary.ENCHANTMENTS =
                  this.$store.state.menu[key].ENCHANTMENTS;
              }
              if (this.$store.state.menu[key].ACTIONS == undefined) {
                this.temporary.ACTIONS = [];
              } else {
                this.temporary.ACTIONS = this.$store.state.menu[key].ACTIONS;
              }
              return;
            } else {
              this.item_name =
  ...
  computed: {
    xy() {
      return [this.$store.state.x, this.$store.state.y, this.$store.state.menu];
    },
  },
};
</script>
